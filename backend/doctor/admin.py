...
class DoctorAvailabilityInline(admin.TabularInline):
    model = DoctorAvailability
    extra = 0
    # Use list comprehension to check for fields dynamically
    readonly_fields = [f.name for f in DoctorAvailability._meta.fields if f.name == 'created_at']

@admin.register(DoctorProfile)
class DoctorProfileAdmin(admin.ModelAdmin):
    list_display = [
        'get_doctor_name', 'specialization', 
        'colored_status', # New: more visible status
        'license_number', 
    ...
    # Ensure actions are correctly registered
    actions = ['approve_doctors', 'reject_doctors', 'reset_to_pending']
    inlines = [DoctorAvailabilityInline]
    ...
    def colored_status(self, obj):
        ...
        return format_html(
            '<b style="color: {};">{}</b>',
            colors.get(obj.verification_status, '#000'),
    ...
    @admin.action(description='✅ Approve selected doctors')
    def approve_doctors(self, request, queryset):
        # Using a loop instead of .update() if you have logic in save()
        count = 0
        for doctor in queryset:
            ...
            doctor.save() # This ensures any signals or save() logic runs
            count += 1
    ...
    def reset_to_pending(self, request, queryset):
        ...
        self.message_user(request, f'🔄 {updated} doctor(s) reset to pending.', messages.INFO)